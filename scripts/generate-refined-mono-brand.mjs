...
const outDir = join(root, 'resources', 'branding', 'refined-mono')
...
function markAsset(fill) {
  return svg({ width: 1024, height: 1024, title: 'Vyotiq mark', content: refinedMark(fill) })
}
...
function lockupAsset(fill, wordmark) {
  ...
  const typeScale = (markSize * 0.44) / wordmark.cap
...
function stackAsset(fill, wordmark) {
  ...
  const markScale = markSize / MARK_SPAN
  const markWidth = MARK_R * Math.sqrt(3) * markScale
  const typeScale = (markWidth * 0.94) / wordmark.box.w
  const typeWidth = wordmark.box.w * typeScale
  const typeHeight = wordmark.box.h * typeScale
  const typeTail = wordmark.tail * typeScale
...
function writeAsset(name, source, width) {
  ...
  writeFileSync(join(outDir, `${name}.png`), rasterize(source, width))
}
...
console.log(`[generate-refined-mono-brand] wrote ${8 * 2} assets to ${outDir}`)

...
function migratePersistedSettingsDefaults(raw: Record<string, unknown>): {
  ...
  // Same contract: rewrite only the exact previous product default (false);
  // a later deliberate off after the v5 stamp is a user choice and survives.
  if (rawVersion < 5 && raw.autoModeSwitch === false) {
    next.autoModeSwitch = DEFAULT_SETTINGS.autoModeSwitch
  }

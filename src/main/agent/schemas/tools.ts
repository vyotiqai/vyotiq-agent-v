    ...
    .array(
      z.object({
        ...
        content: z
          ...
          .optional()
          .describe(
            'Task text (whitespace collapsed); omit on an existing id to update status only (todos are always merged by id)'
          ),
        status: z
          .enum(['pending', 'in_progress', 'completed', 'cancelled'])
          .describe('pending, in_progress, completed, or cancelled')
...
function compactToolNameKey(name: string): string {
  // Invented tool names use any of '_', ' ', or '' as the separator ("Terminal
  // Command", "Terminal_Command", "TerminalCommand"); fold all of them.
...
export function canonicalizeAgentToolName(name: string): string {
  ...
  const alias = TOOL_NAME_ALIASES.get(compact)
  if (alias) return alias
  // A malformed wire payload can pack the whole invocation into the name field
  // (`Terminal Command(command=<arg Key>…`). Real tool names never contain
  // "(" or "<", so retry the lookup chain on the leading tool-name token only.
  const cut = trimmed.search(/[<(]/)
  if (cut > 0) return canonicalizeAgentToolName(trimmed.slice(0, cut))
  return trimmed
}
...
export function formatUnknownToolError(name: string): string {
  ...
  // A malformed wire payload can pack the whole invocation into the name field
  // (`Shell(command=<arg Key>…`); quote only the leading tool-name token.
  const cut = name.search(/[<(]/)
  const bare = cut > 0 ? name.slice(0, cut).trim() : name.trim()
  if (name === 'write_plan') {
    return `Unknown tool "write_plan". Use create_plan to write plan.md.`
  }
  if (/^memory_/i.test(bare)) {
    return `Unknown tool "${bare}". Memory tools are memory_list, memory_read, and memory_write.`
  }
  if (/^todo_/i.test(bare)) {
    return `Unknown tool "${bare}". The run task list tool is todo_write.`
  }
  if (/^(generate_image|edit_image)$/i.test(bare)) {
    return `Unknown tool "${bare}". Image generation is not in the catalog.`
  }
  if (/^(task|agent|subagent)$/i.test(bare)) {
    return `Unknown tool "${bare}". ${catalog} Parallel work uses spawn_agent_instance (root Agent runs).`
  }
  if (/^write$|file_check|create_file|write_file/i.test(bare)) {
    return `Unknown tool "${bare}". Use edit or str_replace to change files.`
  }
  if (/^(bash|shell)$/i.test(bare)) {
    return `Unknown tool "${bare}". The shell tool is terminal.`
  }
  if (/^(web_search|websearch)$/i.test(bare)) {
    return `Unknown tool "${bare}". Web search is browser_search.`
  }
  return `Unknown tool "${bare}". ${catalog}`
}

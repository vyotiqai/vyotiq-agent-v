...
export function toolLabel(
  ...
  // (`Shell(command=<arg Key>…`); reduce to the leading tool-name token.
  ...
  if (effectiveStatus === 'fail') {
    ...
    if (TOOL_LABELS[bare]) return 'Failed'
    return humanizeSnakeCase(bare)
  }
  const labels = TOOL_LABELS[bare]
  if (!labels) {
    const human = humanizeSnakeCase(bare)

...
// agentPersona max 1000, agentTone max 2000, agentIdentity max 1000. The
// briefed renderer constants file (src/renderer/src/features/settings/constants.ts)
// does not exist in this workspace, so the IPC schema limits are authoritative.
...
describe('DEFAULT_AGENT_IDENTITY', () => {
  ...
  it('identity fits the settings length limit (max 1000)', () => {
    expect(DEFAULT_AGENT_IDENTITY.length).toBeLessThanOrEqual(IDENTITY_MAX_LENGTH)
  })

  it('schema caps agentIdentity at 1000 like agentPersona', () => {
    // Mirrors the settings schema: agentIdentity: z.string().max(1000).default('')
    expect(IDENTITY_MAX_LENGTH).toBe(PERSONA_MAX_LENGTH)
    expect(DEFAULT_AGENT_IDENTITY.length).toBeLessThanOrEqual(IDENTITY_MAX_LENGTH)
  })

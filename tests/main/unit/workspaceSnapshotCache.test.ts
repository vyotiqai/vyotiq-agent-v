...
describe('workspace snapshot cache', () => {
  ...
  it('reuses cached listing when workspace fingerprint is unchanged', async () => {
    writeFileSync(join(dir, 'package.json'), '{"name":"demo"}', 'utf8')
    writeFileSync(join(dir, 'alpha.ts'), 'export const a = 1\n', 'utf8')

    const first = await buildWorkspaceSnapshotAsync(dir, 'goal-a')
    const second = await buildWorkspaceSnapshotAsync(dir, 'goal-b')
  ...
  it('invalidates cache when workspace contents change', async () => {
    ...
    const second = await buildWorkspaceSnapshotAsync(dir, 'goal')

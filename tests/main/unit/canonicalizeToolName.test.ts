...
describe('canonicalizeAgentToolName', () => {
  ...
  it('maps common invented aliases onto real tools', () => {
    ...
    expect(canonicalizeAgentToolName('Terminal Command')).toBe('terminal')
  ...
  it('recovers the tool-name token from malformed wire payloads', () => {
    // Real tool names never contain "(" or "<"; a payload that packs the whole
    // invocation into the name field must canonicalize from its leading token.
    expect(
      canonicalizeAgentToolName('Shell(command=<arg Key><arg Value>get-childitem')
    ).toBe('terminal')
    expect(canonicalizeAgentToolName('Terminal(command=<arg Key>…')).toBe('terminal')
    expect(
      canonicalizeAgentToolName('Terminal Command(command=<arg Key><arg Value>ls')
    ).toBe('terminal')
    expect(canonicalizeAgentToolName('Read(path=README.md)')).toBe('read')
  })
...
describe('formatUnknownToolError', () => {
  ...
  it('steers malformed shell payloads at terminal', () => {
    expect(formatUnknownToolError('Shell(command=<arg Key>…')).toMatch(
      /The shell tool is terminal\./
    )
  })

  it('quotes only the tool-name token from malformed payloads', () => {
    expect(formatUnknownToolError('FooBar(command=<arg Key><arg Value>ls')).toMatch(
      /Unknown tool "FooBar"\./
    )
    expect(formatUnknownToolError('FooBar(command=<arg Key><arg Value>ls')).not.toMatch(
      /command/
    )
  })
})

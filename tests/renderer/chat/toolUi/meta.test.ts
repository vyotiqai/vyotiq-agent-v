...
describe('toolUi meta', () => {
  ...
  it('reduces malformed payload names to their tool-name token', () => {
    expect(
      toolLabel('Terminal Command(command=<arg Key><arg Value>get-childitem', 'fail', 'Unknown tool')
    ).toBe('Terminal command')
    expect(toolLabel('Shell(command=<arg Key>…', 'fail', 'Unknown tool')).toBe('Shell')
  })

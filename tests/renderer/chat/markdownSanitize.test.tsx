...
describe('highlighted code output', () => {
  it('escapes html inside a highlighted fence', async () => {
    ...
    // Highlighting is deferred through requestIdleCallback + async shiki imports
    // (core, JS regex engine, theme, grammar). On a cold vitest fork worker
    // that chain can exceed waitFor's 1s RTL default and race the assertion
    // while the plain code shell is still rendered. Widen the window only —
    // pre.shiki must still genuinely appear; the assertions below are unchanged.
    await waitFor(
      () => {
